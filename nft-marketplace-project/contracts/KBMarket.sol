...
contract KBMarket is ReentrancyGuard {
    ...
    function makeMarketItem(address nftContract,uint tokenId,uint price)
        public payable nonReentrant {
            ...
            marketTokenInventory[itemId] = MarketToken(
                itemId,
                nftContract,
                tokenId,
                payable(msg.sender),
    ...
    function createMarketSale(address nftContract,uint itemId) 
        public payable nonReentrant {
            uint price = marketTokenInventory[itemId].price;
            uint tokenId = marketTokenInventory[itemId].tokenId;
            require(msg.value == price, 'Please submit the asking price in order to continue');

            // transfer the amount to the seller
            marketTokenInventory[itemId].seller.transfer(msg.value);
            // transfer the token from contract address to the buyer
            IERC721(nftContract).transferFrom(address(this),msg.sender, tokenId);
            marketTokenInventory[itemId].owner = payable(msg.sender);
            marketTokenInventory[itemId].sold = true;
    ...
    function fetchMarketTokens() public view returns(MarketToken[] memory) {
        ...
        // grabs new instance of MarketToken and set with unsold item count
        MarketToken[] memory items = new MarketToken[](unsoldItemCount);
        for(uint i = 0; i < itemCount; i++) {
            //If it's an unsold item, 0 address
            if(marketTokenInventory[i + 1].owner == address(0)) {
                uint currentId = i + 1;
                MarketToken storage currentItem = marketTokenInventory[currentId];
    ...
    function fetchMyNFTs() public view returns (MarketToken[] memory) {
        ...
        for(uint i = 0; i < totalItemCount; i++) {
            if(marketTokenInventory[i + 1].owner == msg.sender) {
                itemCount += 1;
            }
        }
        ...
        for(uint i = 0; i < totalItemCount; i++) {
            if(marketTokenInventory[i +1].owner == msg.sender) {
                uint currentId = marketTokenInventory[i + 1].itemId;
                // current array
                MarketToken storage currentItem = marketTokenInventory[currentId];
                items[currentIndex] = currentItem;
                currentIndex += 1;
    ...
    function fetchItemsCreated() public view returns(MarketToken[] memory) {
        ...
        for(uint i = 0; i < totalItemCount; i++) {
            if(marketTokenInventory[i + 1].seller == msg.sender) {
                itemCount += 1;
            }
        }
